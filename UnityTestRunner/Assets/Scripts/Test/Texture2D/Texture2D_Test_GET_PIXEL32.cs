...
public class Texture2D_Test_GET_PIXEL32 : MonoBehaviour
{
    ...
    {
        texture = GetComponent<SpriteRenderer>().sprite.texture;
        var colorPixels = texture.GetPixels32();
        int numX;
        int numY;
        ...
        if (index >= 0 && index < colorPixels.Length)
            text.text = "GetPixel32(" + x.text + "," + y.text + ") is " + colorPixels[index].ToString();
    }
}
